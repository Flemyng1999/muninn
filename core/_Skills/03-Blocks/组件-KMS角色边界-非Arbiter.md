...
2. **新建主卡**（`authoritative: true` 的 `03-Zettelkasten/概念-*.md`）
...
| 出口 | 文件 | 谁来 flip |
|---|---|---|
| **seedling 卡草稿** | `03-Zettelkasten/概念-xxx.md` 带 `status: seedling + authoritative: false` | Arbiter flip → `live + authoritative: true` |
...
- 当某次会话涉及 `_Governance/` 或 `03-Zettelkasten/概念-*.md` 改动时，投喂时把本组件和宪法速览一起交给 AI。
